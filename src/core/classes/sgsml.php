...
static function getfile_upload($filename, $start) {
  // 原先程式碼改為透過 $start 輸入變數進行判定如何取得存檔位置與檔案名稱
  // 原則上只有一開始需要由 sys_build_filenam() 取得路徑與唯一檔名, 隨後則沿用至上傳結束
  //list($target,$filename) = sys_build_filename($filename);
  $filename_key = str_replace('.', '_', $filename);
  if($start == 0){
      list($target,$filename) = sys_build_filename($filename);
      $_SESSION[$filename_key]["uploadtarget"] = $target;
      $_SESSION[$filename_key]["uploadfilename"] = $filename;
      self::session_save();
  }else{
      $target = $_SESSION[$filename_key]["uploadtarget"];
      $filename = $_SESSION[$filename_key]["uploadfilename"];
  }
  dirs_checkdir($target);
  $target .= $_SESSION["username"]."__".$filename;
// 原先的檔案儲存方式, 改為透過 $flag 判定新增或附加
/*
...
*/
// file_put_contents() 的特性可以由 $flag 決定新增檔案內容或附加檔案內容
  $flag =($start==0) ? 0:FILE_APPEND;
  if($file_part = file_get_contents("php://input")){
        file_put_contents($target, $file_part, $flag);
        return $target;
    }
  return "";
}

// 配合新增加的 session 變數, 進行存檔, 沒有存檔的 session 值無法使用
