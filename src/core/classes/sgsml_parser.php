...
static function parse_schema($data,$tname,$cache_time,$cache_file) {
  ...
  if (count($tfields)>1 and count($att)>0) {
		  ...
		  if (isset($data_item["VALUES"])) {
			...
			if (isset($data_item["SORT"])) {
			  switch($data_item["SORT"]) {
				case "asc":
				  asort($vals);
				  if (($skey = array_search(sys_remove_trans("{t}Other{/t}"),$vals))) {
					unset($vals[$skey]);
					$vals[$skey] = "{t}Other{/t}";
				  }
				  break;
	...
	foreach (array_keys($tables["views"]) as $key) {
  	...
  	if (!sys_strbegins($tname,"nodb_") and !self::_apply_schema($att["NAME"],$fields)) sys_die("{t}Modifying database failed.{/t}");
  }
...
static function sql_version() {
  ...
  if (SETUP_DB_TYPE=="mysql") {
    $version = mysqli_get_server_info(sys::$db);
...
private static function _apply_indexes($schema_name,$schema_fields) {
  ...
  if (!sql_table_optimize($schema_name)) $buffer[] = sprintf("{t}SQL FAILED: %s{/t}\n","sql_table_optimize").sql_error();
  return $buffer;
}
...
private static function _change_schema_nodb($schema_content,$schema,$is_sys) {
  ...
  if (!preg_match("|<table[^>]*? disable_rights=\"true\".*?>|msi",$schema_content)) {
    $result .= '
	  <view name="rights" displayname="{t}Rights{/t}" schema="sys_nodb_rights" visibility="active" nosinglebuttons="true" noviewbuttons="true" right="admin"></view>
	  <view name="rights_edit" displayname="{t}Edit rights{/t}" schema="sys_nodb_rights_edit" visibility="active" nosinglebuttons="true" noviewbuttons="true" schema_mode="edit" right="admin" template="edit"></view>
    ';
  }
  ...
  if (!strpos($schema_content,'<field name="history"')) $result .= '<field name="history" displayname="{t}History{/t}" simple_type="textarea" notinall="true"><onlyin views="history" /></field>'."\n";

  // needs to be text for smtp (validation would fail)
  if (!strpos($schema_content,'<field name="folder"')) $result .= '<field name="folder" displayname="{t}Folder{/t}" simple_type="folder" simple_default_function="getfolder" hidden="true"></field>';
  ...
  if (!preg_match("|<table[^>]*? disable_schema=\"true\".*?>|msi",$schema_content)) {
    $result .= '
	  <view name="schema" displayname="{t}Schema{/t}" schema="sys_nodb_schema" visibility="bottom" nosinglebuttons="true" noviewbuttons="true" template_mode="flat" right="admin"></view>
    ';
  }
  if (!strpos($schema_content,'<tab name="general"')) $result .= '<tab name="general" displayname="{t}General{/t}" />'."\n";

  if (preg_match("|<table[^>]*? enable_new=\"true\".*?>|msi",$schema_content)) $result = '
	<viewbutton name="paste" displayname="{t}Paste{/t}" onclick="asset_action(\'paste\', 45000);" right="write" accesskey="v"/>
	<view name="edit_as_new" displayname="{t}Edit as new{/t}" template="edit" schema_mode="edit_as_new" nosinglebuttons="true" noviewbuttons="true" right="write" accesskey="r"></view>
	<view name="new" displayname="{t}New{/t}" template="edit" limit="1" default_sql="no_select" schema_mode="new" nosinglebuttons="true" noviewbuttons="true" right="write" accesskey="n"></view>
  '.$result;
  ...
  if (!preg_match("|<table[^>]*? disable_copy=\"true\".*?>|msi",$schema_content)) $result = '
	<viewbutton name="copy" displayname="{t}Copy{/t}" onclick="if (asset_form_selected()) asset_action(\'copy\');" accesskey="c"/>
  '.$result;
  ...
  if ((preg_match("|<table[^>]*? enable_purge=\"true\".*?>|msi",$schema_content) or
	...
	<viewbutton name="cut" displayname="{t}Cut{/t}" onclick="if (asset_form_selected()) asset_action(\'cut\');" right="write" accesskey="x"/>
  '.$result;

  if (preg_match("|<table[^>]*? enable_purge=\"true\".*?>|msi",$schema_content)) $result .= '
	<viewbutton name="purge" displayname="{t}Delete{/t}" onclick="if (asset_form_selected() &amp;&amp; confirm(\'{t}Really delete the dataset(s) ?{/t}\')) asset_action(\'purge\');" right="write" accesskey="d"/>
  ';
  if (preg_match("|<table[^>]*? enable_delete=\"true\".*?>|msi",$schema_content)) $result .= '
	<viewbutton name="delete" displayname="{t}Delete{/t}" onclick="if (asset_form_selected() &amp;&amp; confirm(\'{t}Really delete the dataset(s) ?{/t}\')) asset_action(\'delete\');" right="write" accesskey="d"/>
  ';
  if (preg_match("|<table[^>]*? enable_edit=\"true\".*?>|msi",$schema_content)) {
	...
	$result = '
	  <view name="edit" displayname="{t}Edit{/t}" schema_mode="edit" showinsingleview="true" right="write" enable_calendar="" accesskey="e" '.$addon.'></view>
    '.$result;
  }

  if (preg_match("|<table[^>]*? enable_empty=\"true\".*?>|msi",$schema_content)) $result .= '
	<viewbutton name="empty" displayname="{t}Empty folder{/t}" onclick="if (confirm(\'{t}Really empty the folder ?{/t}\') &amp;&amp; confirm(\'{t}REALLY delete ALL datasets ?{/t}\')) asset_action(\'empty\', 30000);" right="write" accesskey="y"/>
  ';
  if (preg_match("|<table[^>]*? enable_purgeall=\"true\".*?>|msi",$schema_content)) $result .= '
	<viewbutton name="purgeall" displayname="{t}Empty folder{/t}" onclick="if (confirm(\'{t}Really empty the folder ?{/t}\') &amp;&amp; confirm(\'{t}REALLY delete ALL datasets ?{/t}\')) asset_action(\'purgeall\', 30000);" right="write" accesskey="y"/>
  ';
  
  if (!preg_match("|<table[^>]*? disable_search=\"true\".*?>|msi",$schema_content)) {
    $result .= '
	  <view name="search" displayname="{t}Search{/t}" template="display" schema="sys_search" visibility="active" nosinglebuttons="true" noviewbuttons="true" nosqlfolder="true"></view>
    ';
  }
...
private static function _change_schema_types($schema_content) {
  ...
  $schema_content = preg_replace('|(<field[^>]*? simple_type="time"[^>]*?>)(.*?</field>)|msi','\\1<validate function="time"/><store function="datetime_to_int"/><restore function="dateformat||{t}g:i a{/t}"/>\\2',$schema_content);
  $schema_content = preg_replace('|(<field[^>]*? simple_type="datetime"[^>]*?>)(.*?</field>)|msi','\\1<validate function="datetime"/><store function="datetime_to_int"/><restore function="dateformat||{t}m/d/Y g:i a{/t}"/>\\2',$schema_content);
...
private static function _change_schema($schema_content) {
  ...
  if (preg_match("|<table[^>]*? enable_asset_rights=\"full\".*?>|msi",$schema_content)) {
    ...
    $result .= '
	  ...
	  <field name="rread_users" displayname="{t}Read access (users){/t}" simple_type="select" no_search_index="true" simple_default="anonymous" simple_tab="permissions">
	  ...
	  <field name="rwrite_users" displayname="{t}Write access (users){/t}" simple_type="select" no_search_index="true" simple_default="anonymous" simple_tab="permissions">
	    <data title="{t}Users{/t}" function="dbselect|simple_sys_users|username,concat(lastname;\' \';firstname)||lastname asc|10"/>
		<data title="{t}Default{/t}" values="anonymous"/>
  ...
  if (preg_match("|<table[^>]*? enable_asset_rights=\"owner_write\".*?>|msi",$schema_content)) {
    $result .= '
	  <tab name="general" displayname="{t}General{/t}" />
	  <tab name="permissions" displayname="{t}Permissions{/t}" />
	  <field name="rwrite_users" displayname="{t}Write access (users){/t}" simple_type="select" no_search_index="true" simple_default="anonymous" simple_default_function="getusername" simple_tab="permissions">
	    <data title="{t}Users{/t}" function="dbselect|simple_sys_users|username,concat(lastname;\' \';firstname)||lastname asc|10"/>
		<data title="{t}Default{/t}" values="anonymous"/>
		<link value="index.php?find=asset|simple_sys_users||username=@rwrite_users@&amp;view=details"/>
	  </field>
  ...
  if (preg_match("|<table[^>]*? enable_asset_rights=\"owner_read\".*?>|msi",$schema_content)) {
    ...
    $result .= '
	  <tab name="general" displayname="{t}General{/t}" />
	  <tab name="permissions" displayname="{t}Permissions{/t}" />
	  <field name="rread_users" displayname="{t}Read access (users){/t}" simple_type="select" no_search_index="true" simple_default="anonymous" simple_default_function="getusername" simple_tab="permissions">
    	<data title="{t}Users{/t}" function="dbselect|simple_sys_users|username,concat(lastname;\' \';firstname)||lastname asc|10"/>
		<data title="{t}Default{/t}" values="anonymous"/>
		<link value="index.php?find=asset|simple_sys_users||username=@rread_users@&amp;view=details"/>
	  </field>
  ...
  if (!preg_match("|<table[^>]*? disable_history=\"true\".*?>|msi",$schema_content)) {
    $result .= '
	  <view name="history" displayname="{t}History{/t}" visibility="button" template="details" accesskey="i" icon="history.png" show_preview="true"></view>
    ';
  }

  if (!preg_match("|<table[^>]*? disable_structure=\"true\".*?>|msi",$schema_content)) {
    $result .= '
	  <view name="structure" displayname="{t}Structure{/t}" schema="sys_nodb_structure" visibility="bottom" nosinglebuttons="true" noviewbuttons="true" right="admin"></view>
    ';
  }
  if (!preg_match("|<table[^>]*? disable_index=\"true\".*?>|msi",$schema_content)) {
    $result .= '
	  <view name="index" displayname="{t}Index{/t}" schema="sys_nodb_index" visibility="bottom" nosinglebuttons="true" noviewbuttons="true" right="admin"></view>
    ';
  }
