...
// Yen: 2013/08/21 axupload 模組轉化完成, 可以順利在 Firefox 與 Chrome 中上傳多檔與特大檔案.
...
function drop_tree(event) {
  ...
  if (type=="folder" && sys_confirm("Really apply the changes ?")) {
...
function alert_ok(data) {
  sys_alert("Item successfully created.");
}
function search() {
...
function has_flash() {
  ...
  document.write(remove_trans("Flash plugin not installed."));
}
...
function notify(str, warn) {
  ...
  if (sys.menu_autohide) {
	...
	set_html("notification2", remove_trans("Notification: "+str));
  } else {
	set_attr("notification", "class", "menu_notification"+(warn?" red":""));
	set_html("notification", remove_trans("Notification: "+str));
  }
}
...
function refreshit() {
  ...
  ajax("folder_has_changed", [tfolder, sys_time], function(result) {
	if (result) {
	  notify('Folder has changed. <a href="index.php?folder='+escape(tfolder)+'&view='+tview+'">(Refresh)</a>');
	  document.title = remove_trans("(Changed) ") + document.title;
	} else {
...
function searchbox(obj) {
  if (obj.value==remove_trans("Search")) obj.value = "";
  if (obj.value==remove_trans("Other")) obj.value = "";
}

function searchbox2(obj) {
  if (obj.value=="") obj.value = remove_trans("Search");
}
...
function asset_update_confirm(data, id, message) {
  if (typeof(message)=="undefined") message = "Really apply the changes ?";
...
function asset_insert_prompt(fields, field_names) {
  ...
  var info = "\n\nLine break: \\n\nComma: \\k\nSeparate fields: ,\nSeparate values: |\n\n";
  prompt_value = prompt(remove_trans("Quick add: "+field_names.join(", ")+info), prompt_value);
  var data = {};
...
function ajax(func, params, callback, timeout) {
  ...
  var timer = setTimeout(function() {
	timer = null;
	xmlhttp.abort();
	sys_alert("Error doing request to xmlhttp: timeout "+func);
  }, timeout);
    ...
    if (xmlhttp.readyState == 4) {
	  ...
	  try {
		...
		if (result.length > 0) {
		  ...
		  sys_alert("Error: "+error+"\n"+result+"\n"+func+trace);
  } } } }
  xmlhttp.send(JSON.stringify(params));
}

function ajax_binary(func, file, params, callback, callback_progress, item_name) {
// 若網路不通, 則 return 表示終止執行
  if (typeof(navigator.onLine)!="undefined" && !navigator.onLine) return;
// 從 0 byte 開始將 file 以 ajax recursive 的方式上傳
  uploadFileXhr(func, file, params, callback, callback_progress, 0, item_name);
}

function uploadFileXhr(func, file, params, callback, callback_progress, start_byte, item_name)
{
    // totals 為上傳檔案大小
    var totals=file.size;
    // 各小段大小的變數
    var chunk;	
    // 設定上, 每一段預計要分割的 bytes 數, 這裡每一批送 1MB 資料量
    var piece=1*1024*1024 //default 10MB to upload at once
    // 將起始傳送的 start_byte 加上要傳送的 piece 分割 bytes 數, 就是一次傳送的終點  byte
    var end_byte=start_byte+piece;
    // 總共要分成幾段
    var piece_count=end_byte/piece;
    // 判定是否為最後一次傳送
...
var chunk = file.slice(start_byte, end_byte);
reader.readAsBinaryString(chunk);
    ...
    xhr.onreadystatechange=function()
    {
        if (this.readyState == 4) {
            ...
            try {
            if (this.status==200 && result!="") {

                if(is_last==0)
                {
                    // 請注意, 這裡完成一批送檔後, 以 end_byte 作為下一批傳送的 start_byte
                    uploadFileXhr(func, file, params, callback, callback_progress, end_byte, item_name);
                }

              var js = JSON.parse(result);
              if (callback!=null && is_last == 1) callback(js);
            } else sys_alert("Ajax Error: "+func+" "+xmlhttp.status+" "+xmlhttp.statusText+"\n"+(result!=""?result:"no result"));
            } catch (e) {
            if (result.length > 0) {
              if (e == "SyntaxError: parseJSON") e = "";
    ...
    xhr.upload.onprogress=function(e)
    {
        ...
        {
            var percentage  = Math.round((e.loaded+piece_count*piece-piece)*100/totals);
            set_html(item_name+"_progress", (percentage!=100) ? "<br>Uploading: "+file.name+": "+percentage+"%" : "");
        }  
    };

    xhr.open("POST", "ajax.php?function="+escape(func)+"&params="+JSON.stringify(params)+"&start="+start_byte, true);
...
function smenu_home() {
  if (sys.username!="anonymous" && !sys.is_superadmin) {
    smenuitem("Change settings","locate('index.php?find=asset|simple_sys_users|1|username="+sys.username+"&view=changepwd')");
    smenu_hr();
    smenuitem("Home","locate('"+sys.home+"')", "h");
  } else if (sys.is_superadmin) {
    smenuitem("Change settings","locate('index.php?action_sys=edit_setup')");
    smenu_hr();
    smenuitem("Administration","locate('"+sys.home+"')", "h");
    smenuitem("Backup current folder","sWin('index.php?action_sys=backup');");
  } else {
    smenuitem("Home","locate('index.php?folder=1')","h");
  }
}
...
function folder_delete() {
  if (sys_confirm('Really delete ?') && sys_confirm('Really delete ALL subfolders ?')) {
    ajax("folder_delete", [tfolder], locate_folder);
  }
}
function tree_applyrights() {
  if (sys_confirm("Really apply rights to subfolders ?")) {
    ajax("folder_applyrights", [tfolder], function(data) {
      if (data) sys_alert("Item successfully updated.");
	    else sys_alert("Item not found.");
    });
  }
}
...
function drawmenu() {
  ...
  smenu_home();
  smenu_hr();
  smenuitem("Print","neWin('index.php?print=1')");
  smenuitem("Print all","neWin('index.php?print=1&print_all=1')");
  smenu_hr();
  if (tree_visible) smenuitem("Minimize","sWin('index.php?tree=minimize')");
	else smenuitem("Maximize","sWin('index.php?tree=maximize')");
  smenu_hr();
  smenuitem("Collapse tree","sWin('index.php?tree=closeall')");
  smenuitem("Reset view","sWin('index.php?reset_view=true')");
  smenuitem("Close session","sWin('index.php?logout&session_clean')");
  smenu_hr();
  smenuitem("Offline sync","sWin('offline.php?')");
  smenu_end();
  ...
  if (sys.username!="anonymous" && !sys.is_superadmin) {
    ...
    smenu_begin();
	var quick_create = {
	  emails:"E-mail", calendar:"Appointment", tasks:"Task",
	  contactactivities:"Contact activity", contacts:"Contact",
	  passwords:"Password", bookmarks:"Bookmark", notes:"Note",
	  files:"File", gallery:"Photo", spreadsheets:"Spreadsheet",
	  graphviz:"Diagram", htmldocs:"HTML document", cms:"Web page"
	}
  ...
  if (sys.username!="anonymous") {
	smenuitem("Add to offline folders","ajax('folder_add_offline',[tfolder,tview,tfolder_name],alert_ok);");
  }
  if (rights.write_folder && isdbfolder) {
	smenu_hr();
	smenuitem("Move: Up","ajax('folder_moveup',[tfolder],locate_folder)");
	smenuitem("Move: Down","ajax('folder_movedown',[tfolder],locate_folder)");
  }
  if (!no_folder_operations && rights.write_folder) {
	if (isdbfolder) {
	  smenu_hr();
	  smenuitem("Cut","ajax('folder_cut',[tfolder],null)");
	  smenuitem("Copy","ajax('folder_copy',[tfolder],null)");
	  smenuitem("Paste","ajax('folder_paste',[tfolder],locate_folder,30000)");
	}
	smenu_hr();
	smenuitem("Delete incl subfolders","folder_delete()");
  }
  if (rights.admin && isdbfolder) {
	smenu_hr();
	smenuitem("Rights: Show","sWin('index.php?view=rights')");
	smenuitem("Rights: Edit","sWin('index.php?view=rights_edit')","0");
	smenuitem("Apply rights to subfolders","tree_applyrights()");
  }
  if (rights.write_folder && isdbfolder) {
	smenu_hr();
	smenuitem("Merge folders permanently","tree_categories()");
  }
  if (!no_folder_operations && rights.write_folder) {
	smenu_hr();
	smenuitem("Options","tree_folder_options()");
  }
  if (!no_folder_operations && isdbfolder && ((rights.write_folder && !sys.mountpoint_admin) || rights.admin_folder)) {
	smenuitem("Mountpoint","tree_folder_mountpoint()");
  }
  smenuitem("Info","tree_folder_info()");
  smenu_end();
  ...
  smenu_begin();
  if (rights.write) {
	smenuitem("Simple Groupware Import","sWin('import.php?folder="+escape(tfolder)+"')");
	smenu_hr();
  }
  smenuitem("Simple Spreadsheet","neWin('index.php?export=sss')");
  smenuitem("ExtJS Table","neWin('ext/lib/extjs/index.php?title='+escape(tfolder_name)+'&items='+assets_get_selected(true).join(','))");
  smenuitem("HTML","neWin('index.php?export=html')");
  smenuitem("HTML (vertical)","neWin('index.php?export=html_vertical')");
  smenuitem("CSV","sWin('index.php?export=csv')");
  smenuitem("Spreadsheet","sWin('index.php?export=calc')");
  smenuitem("Text Document","sWin('index.php?export=writer')");
  ...
  menubutton("Help","nWin('cms.php?page=Help')");
  menubutton("&nbsp;Login/-out <img src='ext/icons/logout.gif' title='Login/-out'/>&nbsp;","sWin('index.php?logout')", "l", "");
